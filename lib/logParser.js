...
export function analyzeLogs(parsedLogs, fullAnalysis = false) {
  ...
  if (parsedLogs.some((log) => log.timestamp)) {
    ...
    parsedLogs.forEach((log) => {
      ...
      // First try to extract the full datetime in format DD/MMM/YYYY:HH:MM
      let time = "unknown";

      // Try to match format like "24/Feb/2025:09:18:12 +0100"
      const fullDateMatch = log.timestamp.match(
        /(\d{2}\/\w{3}\/\d{4}):(\d{2}:\d{2})/
      );
      if (fullDateMatch) {
        // Use the date and hour:minute parts
        time = `${fullDateMatch[1]} ${fullDateMatch[2]}`;
      } else {
    ...
    metrics.timeDistribution = Object.entries(timeCount)
      ...
      .sort((a, b) => {
        // Try to sort by datetime if possible
        // First check if both are in the format "DD/MMM/YYYY HH:MM"
        const dateTimeRegexA = a.time.match(
          /(\d{2})\/(\w{3})\/(\d{4}) (\d{2}:\d{2})/
        );
        const dateTimeRegexB = b.time.match(
          /(\d{2})\/(\w{3})\/(\d{4}) (\d{2}:\d{2})/
        );

        if (dateTimeRegexA && dateTimeRegexB) {
          // Convert to a sortable format (YYYY-MMM-DD HH:MM)
          const yearA = dateTimeRegexA[3];
          const monthA = dateTimeRegexA[2];
          const dayA = dateTimeRegexA[1];
          const timeA = dateTimeRegexA[4];

          const yearB = dateTimeRegexB[3];
          const monthB = dateTimeRegexB[2];
          const dayB = dateTimeRegexB[1];
          const timeB = dateTimeRegexB[4];

          // Compare years first
          if (yearA !== yearB) return yearA.localeCompare(yearB);

          // Then months (using a month name to number mapping)
          const months = {
            Jan: "01",
            Feb: "02",
            Mar: "03",
          ...
          const monthNumA = months[monthA] || monthA;
          const monthNumB = months[monthB] || monthB;

          if (monthNumA !== monthNumB)
            return monthNumA.localeCompare(monthNumB);

          // Then days
          if (dayA !== dayB) return dayA.localeCompare(dayB);

          // Finally compare times
