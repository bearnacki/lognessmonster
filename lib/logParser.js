...
export function analyzeLogs(parsedLogs, fullAnalysis = false) {
  ...
  if (parsedLogs.some((log) => log.timestamp)) {
    ...
    parsedLogs.forEach((log) => {
      ...
      // Try to extract hour:minute for hourly distribution
      let timeStr = "unknown";
      const timeMatch = log.timestamp.match(/(\d{2}:\d{2}(?::\d{2})?)/);
      if (timeMatch) {
        // Get the original timestamp (HH:MM:SS or HH:MM)
        const originalTimeStr = timeMatch[1];

        // If we have many timestamps, just use the hour portion
        if (shouldGroupByHour) {
          // Extract just the hour part (HH:00)
          timeStr = `${originalTimeStr.split(":")[0]}:00`;
        } else {
          timeStr = originalTimeStr;
        }

        // If we have date information and there might be multiple days,
        // include the date in the time string
        if (date) {
          // Store just time if we'll only have a single day
          // This will be determined later based on the dates.size
          timeCount[`${date}|${timeStr}`] =
            (timeCount[`${date}|${timeStr}`] || 0) + 1;
        } else {
          timeCount[timeStr] = (timeCount[timeStr] || 0) + 1;
        }
      } else if (date) {
        // If we just have date information without time
      ...
      timeDistributionData = Object.entries(timeCount).map(
        ([timeKey, count]) => {
          if (timeKey.includes("|")) {
            ...
            // Format the display value (could be customized further)
            return {
              time: `${formatDateForDisplay(date)} ${time}`,
              fullDate: date,
              timeOnly: time,
              count,
            };
  ...
  function formatDateForDisplay(dateStr) {
    ...
    try {
      ...
      let date;
      if (dateStr.includes("/")) {
        // Format: MM/DD/YYYY or DD/MMM/YYYY
        const parts = dateStr.split("/");
        if (parts[1].length === 3) {
          // DD/MMM/YYYY format
          date = new Date(`${parts[1]} ${parts[0]}, ${parts[2]}`);
        } else {
          // MM/DD/YYYY format (or DD/MM/YYYY depending on locale)
      ...
      // If valid date, format it as a shorter string
