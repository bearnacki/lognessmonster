...
export function analyzeLogs(parsedLogs, fullAnalysis = false) {
  ...
  if (parsedLogs.some((log) => log.timestamp)) {
    ...
    // Track if we have multiple days
    const dates = new Set();

    // Count total timestamps to determine if we need to group by hour
    const totalTimestamps = parsedLogs.filter((log) => log.timestamp).length;
    // If we have many timestamps, group by hour instead of exact minutes
    const shouldGroupByHour = totalTimestamps > 100;

    parsedLogs.forEach((log) => {
      if (!log.timestamp) return;

      // Try to extract date and time information from various formats
      let dateTime = {
        date: null,
        time: null,
      };

      // Handle Apache/Nginx style: [26/Feb/2025:08:48:46 +0100]
      const apacheMatch = log.timestamp.match(
        /\[(\d{2})\/(\w{3})\/(\d{4}):(\d{2}):(\d{2}):(\d{2})\s[+-]\d{4}\]/
      );
      if (apacheMatch) {
        const [_, day, month, year, hours, minutes, seconds] = apacheMatch;
        dateTime.date = `${day}/${month}/${year}`;
        dateTime.time = `${hours}:${minutes}`;
        dates.add(dateTime.date);
      } else {
        // Try to extract date information with more common patterns
        const dateMatch = log.timestamp.match(
          /(\d{4}-\d{2}-\d{2}|\d{2}\/\d{2}\/\d{4}|\d{2}\/\w{3}\/\d{4})/
        );
        if (dateMatch) {
          dateTime.date = dateMatch[1];
          dates.add(dateTime.date);
        }

        // Try to extract time information (hours:minutes)
        ...
        if (timeMatch) {
          ...
          // If we have many timestamps, just use the hour portion
          if (shouldGroupByHour) {
            // Format as HH:00
            dateTime.time = `${hours}:00`;
          } else {
            // Format as HH:MM
            dateTime.time = `${hours}:${minutes}`;
          }
      ...
      // Update the time count based on available information
      if (dateTime.date && dateTime.time) {
        // If we have both date and time
        timeCount[`${dateTime.date}|${dateTime.time}`] =
          (timeCount[`${dateTime.date}|${dateTime.time}`] || 0) + 1;
      } else if (dateTime.time) {
        // If we have only time
        timeCount[dateTime.time] = (timeCount[dateTime.time] || 0) + 1;
      } else if (dateTime.date) {
        // If we have only date
        timeCount[dateTime.date] = (timeCount[dateTime.date] || 0) + 1;
      } else {
        // If we couldn't extract any time information
        timeCount["unknown"] = (timeCount["unknown"] || 0) + 1;
      }
    });

    // Process the time data with consideration for multiple days
    let timeDistributionData;

    if (dates.size > 1) {
      // If we have multiple days, format the entries to show date and time
      timeDistributionData = Object.entries(timeCount).map(
        ([timeKey, count]) => {
          if (timeKey.includes("|")) {
            // Split the date and time parts
            const [date, time] = timeKey.split("|");
            // Format the display value - make sure time is preserved
          ...
          return { time: timeKey, count };
        }
      );
    } else {
      // If we only have one day or no date information, simplify to just times
      timeDistributionData = Object.entries(timeCount).map(
        ([timeKey, count]) => {
          if (timeKey.includes("|")) {
            // Extract just the time part
            const [_, time] = timeKey.split("|");
            return { time, count };
          }
          return { time: timeKey, count };
        }
    ...
    // Sort by date and time if multiple days, otherwise just by time
    metrics.timeDistribution = timeDistributionData.sort((a, b) => {
      if (dates.size > 1 && a.fullDate && b.fullDate) {
        // Sort by date first, then by time
        const dateComparison = a.fullDate.localeCompare(b.fullDate);
        if (dateComparison !== 0) return dateComparison;
        return a.timeOnly?.localeCompare(b.timeOnly) || 0;
      }
      return a.time.localeCompare(b.time);
    });

    // Store whether we have multiple days
    metrics.hasMultipleDays = dates.size > 1;
    // Store whether we grouped by hour
    metrics.timeGroupedByHour = shouldGroupByHour;
  }
  ...
  function formatDateForDisplay(dateStr) {
    ...
    try {
      ...
      // Check for DD/MMM/YYYY format (e.g., 26/Feb/2025)
      const monthAbbrevMatch = dateStr.match(
        /^(\d{1,2})\/([A-Za-z]{3})\/(\d{4})$/
      );
      if (monthAbbrevMatch) {
        const [_, day, monthAbbrev, year] = monthAbbrevMatch;
        // Use the numeric day and the original month abbreviation
        // This avoids localization issues
      ...
      if (dateStr.includes("-")) {
        ...
        if (parts.length === 3) {
          // Just use first two parts (day/month)
          formattedDate = `${parts[0]}/${parts[1]}`;
        }
      }

      // Return a simplified date format (day/month)
      if (formattedDate) {
        return formattedDate;
      }

      // Fallback to the original string if we couldn't parse it
      return dateStr;
    } catch (e) {
